...
const regions = [
  { name: "FastAPI + Uvicorn", nodes: 1, status: "healthy" },
  { name: "PostgreSQL 15", nodes: 1, status: "healthy" },
  { name: "Redis 7", nodes: 1, status: "healthy" },
  { name: "Nginx", nodes: 1, status: "healthy" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                A single Docker Compose stack runs Postgres, Redis, FastAPI, and Nginx.
                Non-root containers, healthchecks everywhere, migrations on start.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                ...
                <span className="text-2xl text-muted-foreground">services</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Postgres, Redis, FastAPI, and Nginx orchestrated together — with healthchecks on every service and `alembic upgrade head` on container start.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">60/min</span>
              <span className="block text-sm text-muted-foreground mt-2">Rate limit (SlowAPI)</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">/health</span>
              <span className="block text-sm text-muted-foreground mt-2">Load balancer probe</span>
            </div>
          </div>
        </div>
